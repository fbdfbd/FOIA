...
public class TMPFontReplacer : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        if (GUILayout.Button($"폰트 크기 +1 ({foundTexts.Count}개)", GUILayout.Height(32)))
        {
            IncreaseFontSizes(1f);
        }
    ...
    private void IncreaseFontSizes(float amount)
    {
        if (foundTexts.Count == 0)
        {
            FindAllTMPTextsInScene();
        ...
        Undo.IncrementCurrentGroup();
        Undo.SetCurrentGroupName("Increase TMP Font Sizes");
        int undoGroup = Undo.GetCurrentGroup();
        ...
        foreach (var text in foundTexts)
        {
            ...
            Undo.RecordObject(text, "Increase TMP Font Size");
        ...
        Debug.Log($"[TMPFontReplacer] {changed}개의 TMP_Text 폰트 크기를 +{amount} 했습니다. " +
                  $"(프리팹 오버라이드 발생: {prefabOverride}개)");
        EditorUtility.DisplayDialog("완료",
            $"{changed}개 TMP_Text 폰트 크기 +{amount} 완료!\n프리팹 오버라이드: {prefabOverride}개",
            "확인");
    }
}
